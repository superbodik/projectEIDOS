    ...
    for (int x = 0; x < CHUNK_SIZE_X; x++) {
        for (int y = 0; y < CHUNK_SIZE_Y; y++) {
            for (int z = 0; z < CHUNK_SIZE_Z; z++) {
                ...
                // --- ФИКС ПОРЯДКА ВЕРШИН (Winding Order) ---
                // Треугольники должны быть определены ПРОТИВ часовой стрелки (CCW).
                // Иначе они считаются "смотрящими назад" и не рисуются.
                ...
                // FRONT (+Z) [SOUTH] - ПЕРЕПИСАН ПОРЯДОК
                ...
                // BACK (-Z) [NORTH] - ПЕРЕПИСАН ПОРЯДОК
                ...
                // RIGHT (+X) [EAST] - ПЕРЕПИСАН ПОРЯДОК
